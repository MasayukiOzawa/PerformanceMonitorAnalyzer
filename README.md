...
- **☑️ PerfMon 風選択**: オブジェクト、カウンター、インスタンスを選んで追加済みカウンターを管理
...
- 標準のパフォーマンスモニターに近い形式で、オブジェクト、カウンター、インスタンス、追加済みカウンターを分けて表示
- オブジェクトを選択し、カウンターとインスタンスをチェックして「追加(>>)」を押すことで、読み込み対象のカウンター一覧に追加
- `Process` など複数インスタンスを持つオブジェクトでは、インスタンス一覧の `<すべてのインスタンス>` から一括追加が可能
- 追加済みカウンターは「削除(<<)」または「クリア」で読み込み対象から除外可能
...
3. **カウンター選択**（以下のいずれかの方法）：
   ...
   - **手動選択**: 左側の PerfMon 風選択UIで、オブジェクト、カウンター、インスタンスを選んで「追加(>>)」をクリック
