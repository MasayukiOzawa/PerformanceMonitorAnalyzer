...
2. **ファイル > BLGファイルを開く（relog.exe使用）** - より安定したrelog.exe を使用した方法
3. **ファイル > サンプルBLGファイルを読み込み（PDH API）** - 付属のサンプルファイルをPDH APIで読み込み
4. **ファイル > サンプルBLGファイルを読み込み（relog.exe）** - 付属のサンプルファイルをrelog.exe で読み込み
...
- **データ形式**: JSON (Newtonsoft.Json), CSV
- **BLG解析**: 
