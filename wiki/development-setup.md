...
└── PerformanceMonitorAnalyzer/
    ...
    ├── PdhApi.cs
    ├── CounterPattern.cs
    ├── ChartType.cs / CounterValueMode.cs / NodeType.cs
    ├── LegendItem.cs / CounterTreeNode.cs / PerformanceDataPoint.cs
    ├── CounterStatistics.cs / CounterStatisticsItem.cs
    └── LogEntry.cs / LogLevel.cs
```

- `MainWindow.xaml.cs` は画面ロジックに集中させ、補助的な enum / モデル / ログエントリは個別ファイルへ分離しています。
...
4. **境界コントロール**（Row 3）: リサイズ用の分割線と下部エリア開閉トグル
5. **データテーブル / ログ表示**（Row 4）: タブ形式のデータ表示とログ表示

### 動的リサイズ機能
- Row 3 に配置された `GridSplitter` により、データテーブル / ログ表示領域の高さを動的に調整可能
- 同じ Row 3 の中央トグルボタンにより、下部エリア全体を最小化 / 再表示可能
- 最小100ピクセル、最大750ピクセルの制約あり
- マウスオーバー時の視覚的フィードバック機能

```xml
<GridSplitter Grid.Row="3"
              x:Name="BottomPanelGridSplitter"
              Height="22"
              ResizeDirection="Rows"
              ResizeBehavior="PreviousAndNext"
              ToolTip="ドラッグしてデータテーブル / ログ表示エリアの高さを調整"/>
<Button Grid.Row="3"
        x:Name="BottomPanelToggleButton"
        Click="BottomPanelToggleButton_Click"
        Content="▼"/>
```
