...
Performance Monitor Analyzer は、Windows Performance Monitor の .blg ファイルを読み込み、解析・可視化するC#アプリケーションです。

**🎉 NEW: MAUI版リリース!** クロスプラットフォーム対応により、Windows、macOS、iOS、Androidで利用可能になりました。
...
### ナレッジベース
- [🚀 WPFからMAUIへの移行](../knowledge/42-WPFからMAUIへの移行.md) - クロスプラットフォーム対応実装詳細（NEW）
