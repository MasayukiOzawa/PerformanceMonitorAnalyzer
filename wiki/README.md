...
- [🚀 2並列でのカウンターの読み込み](../knowledge/58-2並列でのカウンターの読み込み.md) - カウンター読み込み処理の2並列化とプロット処理の直列化によるパフォーマンス向上と安定性確保（メモリ制御、エラーハンドリング強化）
...
- **BLGファイル読み込み**: Windows Performance Monitor のバイナリログファイルを解析
  ...
  - **🚀 2並列カウンター読み込み**: SemaphoreSlim(2,2)による効率的な並列処理とデッドロック回避
