...
**追加要件（コメント要求）**: 
1. BLGファイルに含まれる時間範囲を活用し、スライダーバーで特定の時間範囲のみ選択してrelog実行できる機能の実装
2. **relog実行状況の詳細表示** ⭐追加要求
3. **手動実行ボタンの追加** ⭐追加要求  
4. **CSVデータを基にしたタブ表示とデータテーブル出力** ⭐追加要求
...
**relog実行状況表示UI要素を追加** ⭐新機能:
- `RelogStatusGroup`: relog実行状況表示グループボックス
- `CurrentProcessingCounter`: 現在処理中のカウンター名表示
- `RelogProgressText`: 進捗詳細メッセージ表示
- `ExecuteRelogButton`: 手動実行ボタン（選択されたカウンターを一括処理）
...
## ⭐追加新機能: relog実行状況表示と手動実行

### relog実行状況表示UI
```xml
<!-- relog実行状況グループボックス -->
<GroupBox x:Name="RelogStatusGroup" Header="🔄 relog実行状況" Visibility="Collapsed">
    <!-- 現在処理中のカウンター -->
    <TextBlock x:Name="CurrentProcessingCounter" Text="処理中のカウンター: なし" />
    
    <!-- 進捗詳細 -->
    <TextBlock x:Name="RelogProgressText" Text="待機中..." />
    
    <!-- 手動実行ボタン -->
    <Button x:Name="ExecuteRelogButton" Content="🚀 選択されたカウンターでrelog実行" 
            IsEnabled="False" />
</GroupBox>
```
...
{
    if (checkBox.Tag is string counter)
    {
        // 選択されたカウンターリストに追加
        ...
        // 自動でデータ読み込みを実行
        _ = Task.Run(async () => await LoadSingleCounterDataAsync(counter));
    }
}
```
